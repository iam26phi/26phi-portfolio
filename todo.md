...
### 目標
優化照片上傳體驗，新增縮圖預覽、進度條優化和拖放上傳支援

### 功能清單
- [x] 實作拖放上傳功能（Drag & Drop）
  - 拖曳檔案到頁面時顯示拖放區域
  - 支援多檔案拖放
  - 拖放區域視覺提示（漸層背景 + 虛線框）
- [x] 新增上傳預覽縮圖
  - 顯示正在上傳的照片縮圖（16x16 像素）
  - 縮圖旁顯示檔案名稱和大小（MB）
  - 上傳完成後顯示成功標記（綠色勾選）
  - 失敗時顯示錯誤標記（紅色 X）
- [x] 優化進度條顯示
...
### 目標
1. 將照片管理頁面的標題固定在頂部
2. 將 InlineToggle 按鈕改為下拉選單，避免誤觸

### 實施步驟
- [x] 修改 Admin.tsx 標題區域為固定定位
- [x] 修改 InlineToggle 組件為下拉選單
- [x] 測試修改效果

### 實際效果
- 標題固定：使用 sticky 定位，滾動時標題始終可見
- InlineToggle 改為下拉選單：點擊顯示選項，避免誤觸
- 提升使用者體驗和操作安全性
