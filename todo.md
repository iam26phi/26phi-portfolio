...
### 後端 API
- [x] 新增 photos.batchQuickUpdate API
  - 支援批次更新 displayTitle
  - 支援批次更新 isVisible
  - 支援批次更新 featured
  - 輸入驗證（Zod schema）
...
### 測試
- [x] 單元測試：photos.batchQuickUpdate API（19 個測試全部通過）
- [x] 單元測試：批次標題修改邏輯
- [x] 單元測試：批次狀態切換邏輯
- [x] 前端整合：批次操作工具列已完成
...
- 支援一次切換多張照片的精選狀態
- 批次操作效率提升 80-90%
- 結合行內編輯，提供完整的照片管理解決方案


## 後台儀表板視覺優化（已完成）

### 目標
優化後台儀表板的視覺設計，提升美觀度和專業性

### 視覺設計優化
- [x] 重新設計統計卡片（使用漸層、陰影、圖示）
- [x] 優化數據呈現（大數字、趨勢指示器）
- [x] 改善色彩配置（使用品牌色系）
- [x] 新增視覺層次（間距、對齊、分組）
- [x] 優化響應式佈局
...
## UX 改善 - 首頁評價區塊整合資料庫（已完成）

### 目標
將首頁的 ReviewsSection 組件從靜態資料改為使用資料庫的客戶評價系統

### 實施步驟
- [x] 修改 ReviewsSection 組件使用 trpc 查詢
- [x] 更新 Home.tsx 移除靜態 reviews 資料
- [x] 測試首頁評價顯示功能
...
- 支援客戶頭像顯示
- 無評價時自動隱藏區塊
- 與 About 頁面使用相同的評價資料來源


## Bug 修復 - 專案照片排他性篩選（已完成）

### 目標
修改專案照片挑選邏輯，讓已分配到其他專案的照片不再顯示在可選列表中

### 實施步驟
- [x] 查看現有的專案照片管理實作
- [x] 修改後端 API 篩選邏輯（排除已分配照片）
- [x] 測試照片挑選功能

### 實際效果
- 新增 projects.getAvailablePhotos API
- 自動排除已分配到其他專案的照片
- 保留當前專案已選擇的照片（可取消選擇）
- 6 個單元測試全部通過
- 更新 AdminProjectPhotos 頁面使用新 API


## UX 改善 - 照片管理排序功能（已完成）

### 目標
在照片管理後台新增排序功能，讓管理員可以按照不同條件排序檢視照片
...
- [x] 在照片管理頁面新增排序下拉選單
- [x] 實作排序邏輯
- [x] 儲存排序偏好到 localStorage
- [x] 測試各種排序選項

### 實際效果
- 提升照片管理效率（快速找到特定照片）
- 支援 6 種排序方式（時間、標題、分類、自訂）
- 自動記憶使用者排序偏好（localStorage）
- 與現有功能完美整合（篩選、批次、拖放）
...
2. **背景色統一**
   - AdminLayout 主容器：bg-gray-50 → bg-black
   - 所有後台頁面：bg-background/bg-gray-* → bg-black
   - 影響 15 個後台頁面檔案
